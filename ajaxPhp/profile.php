<?php
	...
	switch($_POST['action']) {
		case 'getProfile':
			$statement = $database->prepare("SELECT fullname, email FROM user WHERE user_id = :id");
			...
			break;
		case 'editProfile':
			//checking if data needed data is not empty.
			if(empty($_POST['fullname'])) {
				die('incomplete_data');
			}
			
			//updating profile
			$statement = $database->prepare("UPDATE user SET fullname = :fullname WHERE user_id = :id");
		...
		case 'editPassword':
			...
			$statement = $database->prepare("UPDATE user SET password = :password WHERE user_id = :id");
		...
		case 'editEmail':
			...
			if($accountVerification->getStatus() == 'SEND CODE') {
				...
				$statement = $database->prepare("UPDATE user SET email = :email WHERE user_id = :id");
	...
	function isPasswordCorrect($password) {
		global $database;
		$statement = $database->prepare("SELECT password FROM user WHERE user_id = :id");
		$statement->bindParam(':id', $_SESSION['USER_INFORMATION']['ID']);
		if(!$statement->execute()) {
			die('error');
		}
